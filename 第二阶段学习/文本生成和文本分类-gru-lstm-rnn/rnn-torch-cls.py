...
class Model(torch.nn.Module):
    def __init__(self, vocab_size,input_emb_dim, hidden_size, label_nums):
        ...
        self.rnn = torch.nn.RNN(input_emb_dim, hidden_size, batch_first = True)
    ...
    def forward(self, batch_input_ids):
        ...
        '''
        _, last_hidden = self.rnn(input_emb)    # 1 * batch_size * hidden_size
        cls = self.cls(last_hidden).squeeze(0)             # 1 * batch_size * label_nums
        return  cls
